...
async function testGraphsDisplay(driver) {
  try {
    ...
    await driver.get("http://localhost:5500/dfps.html");
    ...
    // Check if the streamflow plot is displayed
    const streamflowPlot = await driver.findElement(By.id("streamflowPlot"));
    ...
    // Submit form to generate graphs
    const form = await driver.findElement(By.id("data_form"));
    await form.submit();

    // Wait for plots to be generated
    await driver.wait(
      until.elementLocated(By.css("#streamflowPlot .plot-container")),
      5000
    );
    await driver.wait(
      until.elementLocated(By.css("#qFractionPlot .plot-container")),
      5000
    );

    // Verify plots contain data
    const streamflowPlotData = await driver.findElement(
      By.css("#streamflowPlot .plot-container")
    );
    const qFractionPlotData = await driver.findElement(
      By.css("#qFractionPlot .plot-container")
    );

    console.log(
      "Streamflow plot contains data:",
      await streamflowPlotData.isDisplayed()
    );
    console.log(
      "qFraction plot contains data:",
      await qFractionPlotData.isDisplayed()
    );
  } catch (error) {
