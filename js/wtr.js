...
//check for input errors address them as needed with messages or defaults
    ...
    let rDay = (pYR) / 365.25/ 1000
    //calculate flow at left
    let qLeft = (kH * (hLt * hLt - hRt * hRt) / 2 / distL) - 
        (rDay * (distL / 2 - 0));
        //calculate flow at top
    let qTop = rDay * distL;
    //calculate flow at right
    let qRight = (kH * (hLt * hLt - hRt * hRt) / 2 / distL) - 
        (rDay * (distL / 2 - distL));
    ...
    let inner_SQRT = (hLt * hLt) - ((hLt * hLt - hRt * hRt) * divide / distL) + 
    (rDay / kH) * (distL - divide) * divide;
    // hMax at location adjusted from outside of land mass
    let inner_SQRToutside = (hLt * hLt) - ((hLt * hLt - hRt * hRt) * divideAdjusted / distL) + 
    ...
    let result_Rch = "<font color=green><b>Recharge Rate (Precipitation-Runoff-Evapotranspiration) &nbsp&nbsp&nbsp&nbsp mm/YR = " + mmYR +
    ...
    let graph_caption = "<font color=black><font size=3px>The results are plotted below</font size>";
