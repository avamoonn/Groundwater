...
describe("Stream Flow Calculations", () => {
  describe("calculateStreamLeakage", () => {
    test("should correctly calculate stream leakage for typical values", () => {
      const Qw = 0.0333; // 2000 L/min converted to m³/s
      const Qfraction = 0.5;
      const expected = 0.01665; // 0.0333 * 0.5
      const result = calculateStreamLeakage(Qw, Qfraction);
      expect(result).toBeCloseTo(expected, 5);
    });

    test("should handle zero pumping rate", () => {
      const Qw = 0;
      const Qfraction = 0.5;
      const result = calculateStreamLeakage(Qw, Qfraction);
      expect(result).toBe(0);
    ...
    test("should handle complete stream depletion", () => {
      const Qw = 0.0333;
      const Qfraction = 1.0;
      const result = calculateStreamLeakage(Qw, Qfraction);
      expect(result).toBeCloseTo(0.0333, 5);
    });
  });

  describe("calculateStreamDischarge", () => {
    test("should correctly calculate remaining stream discharge", () => {
      const Qs = 3.0; // Initial stream discharge (m³/s)
      const QstreamLeakage = 0.01665; // Stream leakage (m³/s)
      const expected = 2.98335; // 3.0 - 0.01665
      const result = calculateStreamDischarge(Qs, QstreamLeakage);
      expect(result).toBeCloseTo(expected, 5);
    });

    test("should handle zero leakage", () => {
      const Qs = 3.0;
      const QstreamLeakage = 0;
      const result = calculateStreamDischarge(Qs, QstreamLeakage);
    ...
    test("should handle complete stream depletion", () => {
      const Qs = 0.0333;
      const QstreamLeakage = 0.0333;
      const result = calculateStreamDischarge(Qs, QstreamLeakage);
      expect(result).toBeCloseTo(0, 5);
    });
  });

  describe("Stream Flow Integration Tests", () => {
    test("should correctly calculate stream depletion over time", () => {
      // Example values from the default form inputs
      const d = 50; // meters
      const Sy = 0.15;
      const Ka = 0.001; // cm/s
      const b = 10; // meters
      ...
      // Calculate for a specific time point (e.g., 1 day)
      const time = 1;
      const Qfraction = calculateQFraction(d, Sy, T, time);
      const QstreamLeakage = calculateStreamLeakage(Qw, Qfraction);
      const remainingDischarge = calculateStreamDischarge(Qs, QstreamLeakage);

      // Verify results are within expected ranges
      expect(Qfraction).toBeGreaterThan(0);
      expect(Qfraction).toBeLessThan(1);
      expect(QstreamLeakage).toBeGreaterThan(0);
      expect(QstreamLeakage).toBeLessThan(Qw);
      expect(remainingDischarge).toBeLessThan(Qs);
      expect(remainingDischarge).toBeGreaterThan(0);
    });
  });
});
